...
#include "esp_log.h"
#include "sdkconfig.h"
#include "modbus_requests_generated.h"
...
namespace modbus
{
    ...
    class ModbusRequest
    {
        ...
        ModbusError DoBasicChecks(uint8_t *buf, size_t &bufLen, uint8_t slaveId, FunctionCodes functionCode, uint8_t expectedByteCount){
            if(buf[0]!=slaveId){
                ESP_LOGE(TAG, "ModbusError::WRONG_SLAVE_ID_IN_RESPONSE is=%d should=%d", buf[0], slaveId);
                return ModbusError::WRONG_SLAVE_ID_IN_RESPONSE;
            }
            if((buf[1] & 0x7F) != (uint8_t)functionCode){
                ESP_LOGE(TAG, "ModbusError::WRONG_FUNCTION_CODE_IN_RESPONSE is=%d should=%d", buf[1], (uint8_t)functionCode);
                return ModbusError::WRONG_FUNCTION_CODE_IN_RESPONSE;
            }
            if(buf[1] & 0x80){
                ESP_LOGE(TAG, "ModbusError::SLAVE_REPORTED_PROCESSING_ERROR");
                return ModbusError::SLAVE_REPORTED_PROCESSING_ERROR;
            }
            if(buf[2] != expectedByteCount){
                ESP_LOGE(TAG, "ModbusError::WRONG_PAYLOAD_SIZE is=%d should=%d", buf[2], expectedByteCount);
                return ModbusError::WRONG_PAYLOAD_SIZE;
            }
            if(bufLen!=5+buf[2]){
                ESP_LOGE(TAG, "ModbusError::WRONG_BUFFER_SIZE is=%d should=%d", bufLen, 5+buf[2]);
                return ModbusError::WRONG_BUFFER_SIZE;
            }
            if(!validCRCInLastTwoBytes(buf, bufLen)){
                ESP_LOGE(TAG, "ModbusError::CRC_ERROR");
    ...
    public:
        ...
        ModbusError ProcessResponse(uint8_t *buf, size_t& bufLen) override{
            ModbusError err;
            if((err=DoBasicChecks(buf, bufLen, slaveId, FunctionCodes::READ_INPUT_REGISTERS, (count+7)/8))!=ModbusError::OK){
                return err;
            }
    ...
    public:
        ...
        ModbusError ProcessResponse(uint8_t *buf, size_t& bufLen) override{
            ...
            ModbusError err;
            if((err = DoBasicChecks(buf, bufLen, slaveId, FunctionCodes::READ_INPUT_REGISTERS, 2*count))!=ModbusError::OK){
                return err;
            }
            ...
            while (true)
            {
                ...
                if (!xQueueReceive(uart_queue, (void *)&event, ticks))
                {
                    ...
                    if(newNowUs<nextTimeoutUs){
                        ESP_LOGW(TAG, "Timeout was not long enough previousNow=%d, nextTimeoutUs=%d, timeoutUs=%d, ticks=%d, newNow=%d", (uint32_t)nowUs, (uint32_t)nextTimeoutUs, (uint32_t)timeoutUs, (uint32_t)ticks, (uint32_t)newNowUs);
                        continue;
                    }
                    nowUs=newNowUs;
                    switch (state)
                ...
                else if (event.type == UART_DATA && event.timeout_flag)
                {
                    ...
                    ModbusError err = this->requests->at(lastRequestIndex)->ProcessResponse(dtmp, event.size);
